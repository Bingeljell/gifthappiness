...
const worker = {
  // Every response is routed through withCors so the Access-Control-Allow-Origin
  // header reflects *this* caller's origin. Route handlers build responses via
  // json()/errorResponse(), which don't hold the Request and so can only emit
  // the default origin -- correcting it once here beats threading the Request
