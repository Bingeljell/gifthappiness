...
1. **Stage 1 — unified accounts + host dashboard.** Schema migration (`users`, `sessions`, `contributions.donor_id`, `verifications` purpose constraint) on the live Supabase project; `/auth/*` routes; `GET /me/celebrations`; `/sign-in` and `/account` pages; `Header.tsx` wiring.
2. **Stage 2 — donor history.** `/celebration`'s contribution form attaches `donor_id` when signed in; `GET /me/contributions`; a "your past donations" section on `/account`.
3. **Stage 3 — admin RBAC.** `is_admin` flag, `/admin/*` routes switch to session+`isAdmin` checks, a minimal admin landing page. `ADMIN_API_KEY` can stay as a fallback during the transition rather than being deleted outright.
