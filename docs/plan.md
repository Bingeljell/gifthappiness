...
1. **Stage 1 — unified accounts + host dashboard.** [x] Schema migration (`users`, `sessions`, `contributions.donor_id`, `verifications` purpose constraint) applied to the live Supabase project. [x] `/auth/*` routes (`request`, `confirm`, `me`, `logout`) and `GET /me/celebrations`, deployed and verified live end-to-end via curl (login code issued → brute-forced from the stored hash, same technique as the earlier host-verification test → confirmed → session validated → celebrations listed → logout invalidates the session). [x] `/sign-in` and `/account` pages, `Header.tsx` wiring -- `npm run build`/`npm run lint` pass; browser-verified through the request-code step only. **Resume point:** click through the confirm-code → `/account` flow in the browser (the code paths are typechecked against already-curl-verified endpoints, just not clicked through yet), then this Worker deploy already includes the new routes, but a *second* redeploy happened afterward only to flip `ALLOWED_ORIGIN` for that browser test and flip it back -- no further backend deploy needed to resume, just the frontend click-through.
